...
TEST(ChooseStatePrecision, AmpleVramKeepsFp32) {
    const ses::PrecisionChoice c = choose_state_precision(
        16 * kGiB, kNumStates, kBytesPerStateFp32, kHeadroom);
    EXPECT_EQ(c.p, GpuPrecision::Fp32);
    EXPECT_TRUE(c.fits);
}
...
TEST(ChooseStatePrecision, Fp32OverflowsButFp16Fits) {
    ...
    const ses::PrecisionChoice c = choose_state_precision(
        8 * kGiB, kNumStates, kBytesPerStateFp32, kHeadroom);
    EXPECT_EQ(c.p, GpuPrecision::Fp16);
    EXPECT_TRUE(c.fits);
}
...
TEST(ChooseStatePrecision, EvenFp16OverflowsStillPicksFp16AndFlags) {
    ...
    const ses::PrecisionChoice c = choose_state_precision(
        4 * kGiB, kNumStates, kBytesPerStateFp32, kHeadroom);
    EXPECT_EQ(c.p, GpuPrecision::Fp16);
    EXPECT_FALSE(c.fits);
}
...
TEST(ChooseStatePrecision, UnmeasurableBudgetDefaultsToFp32) {
    ...
    const ses::PrecisionChoice c = choose_state_precision(
        ses::kVramUnknown, kNumStates, kBytesPerStateFp32, kHeadroom);
    EXPECT_EQ(c.p, GpuPrecision::Fp32);
    EXPECT_TRUE(c.fits);
    // Any negative (nonsense) free value: same default.
    const ses::PrecisionChoice neg = choose_state_precision(
        -5, kNumStates, kBytesPerStateFp32, kHeadroom);
    EXPECT_EQ(neg.p, GpuPrecision::Fp32);
    EXPECT_TRUE(neg.fits);
}
