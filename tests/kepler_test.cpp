// RED: Kepler circular-pair seed. Under E_n=-1/(2n^2) the angular coherence
// <e^{i phi}> is localized and orbits CCW at the Kepler rate ~1/n_bar^3.
...
TEST(KeplerSeed, CircularPairTableMatchesTheManifold) {
    ...
    for (std::size_t i = 0; i < pairs.size(); ++i) {
        ...
        EXPECT_EQ(sc.l, p.n - 1);
        EXPECT_EQ(sc.m, sc.l);
        EXPECT_EQ(ss.level, sc.level);
        EXPECT_EQ(ss.m, -sc.l);
...
TEST(KeplerSeed, PacketOrbitsCcwAtTheKeplerRate) {
    ...
    const auto pairs = ses_shell::kepler_pairs();
    // <e^{i phi}> ~ sum_n a_{n+1}^* a_n, a_n(t) = a_n e^{-i E_n t}, E_n = -1/(2 n^2).
